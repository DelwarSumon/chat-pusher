$(function () {
    $('#action_menu_btn').click(function () {
        $(this).closest('.card').find('.action_menu').toggle();
    });
    ...
    $(".chat-toggle").on("click", function (e) {
        ...
        cloneChatBox(user_id, username, function () {
            ...
            if (!chatBox.hasClass("chat-opened")) {
                ...
                chatBox.find(".chat-area").animate({ scrollTop: chatBox.find(".chat-area").scrollTop() + chatBox.find(".chat-area").outerHeight(true) }, 800, 'swing');
            }
        });
    });
...
function getMessageSenderHtml(message) {
    ...
    //          <img src="` + base_url +  '/asset_custom/images/user-avatar.png' + `" width="50" height="50" class="img-responsive">
    //      </div>
    //  </div>
    //  `;
    ...
    return `
        ...
        <div class="img_cont_msg avatar">
            <img src="` + base_url + '/asset_custom/images/user-avatar.png' + `" class="rounded-circle user_img_msg">
        </div>
        <div class="msg_cotainer messages msg_sent">
            ${message.content}
            <span class="msg_time"><time datetime="${message.dateTimeStr}"> ${message.fromUserName} • ${message.dateHumanReadable}</time></span>
        </div>
    </div>
    `;
...
function getMessageReceiverHtml(message) {
    ...
    return `
        <div class="d-flex justify-content-end mb-4 msg_container base_receive" data-message-id="${message.id}">
            ...
            ${message.content}
                <span class="msg_time_send"><time datetime="${message.dateTimeStr}"> ${message.fromUserName}  • ${message.dateHumanReadable}</time></span>
            </div>
            <div class="img_cont_msg avatar">
                <img src="` + base_url + '/asset_custom/images/user-avatar.png' + `" class="rounded-circle user_img_msg">
            </div>
...
function send(to_user, message) {
    ...
    $.ajax({
        ...
        complete: function () {
            ...
            chat_area.animate({ scrollTop: chat_area.scrollTop() + chat_area.outerHeight(true)  }, 800, 'swing');
        }
    });
}
/**
